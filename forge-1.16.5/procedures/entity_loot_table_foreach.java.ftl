...
<#-- @formatter:off -->
if (${input$entity} instanceof LivingEntity _entity && !_entity.world.isClientSide() && _entity.getServer() != null) {
	...
	for (ItemStack itemstackiterator : _entity.getServer().getLootTables().get(${toResourceLocation(input$location)})
			.getRandomItems(new LootContext.Builder((ServerWorld) _entity.world)
