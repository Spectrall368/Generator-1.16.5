<#if field$dimension??><#--Here for legacy reasons as field$dimension does not exist in older workspaces-->
{
	...
	if(!_ent.world.isRemote&&_ent instanceof ServerPlayerEntity) {
		...
		if (nextWorld != null) {
			...
			((ServerPlayerEntity) _ent).teleport(nextWorld, ((ServerPlayerEntity) _ent).getX(), ((ServerPlayerEntity) _ent).getY(), ((ServerPlayerEntity) _ent).getZ(), _ent.rotationYaw, _ent.rotationPitch);
