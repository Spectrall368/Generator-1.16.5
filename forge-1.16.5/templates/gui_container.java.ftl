...
public class ${name}Menu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	<#if data.type == 1>
		@Override public ItemStack transferStackInSlot(PlayerEntity playerIn, int index) {
			...
			if (slot != null && slot.hasItem()) {
				...
				if (index < ${slotnum}) {
					if (!this.moveItemStackTo(itemstack1, ${slotnum}, this.inventorySlots.size(), true)) {
						return ItemStack.EMPTY;
					}
					slot.onSlotChange(itemstack1, itemstack);
				} else if (!this.mergeItemStack(itemstack1, 0, ${slotnum}, false)) {
					if (index < ${slotnum} + 27) {
						if (!this.moveItemStackTo(itemstack1, ${slotnum} + 27, this.inventorySlots.size(), true)) {
							return ItemStack.EMPTY;
						}
					} else {
		...
		@Override ${mcc.getMethod("net.minecraft.inventory.container.Container", "mergeItemStack", "ItemStack", "int", "int", "boolean")
			.replace("slot.setChanged();", "slot.putStack(itemstack);")
		...
		private void slotChanged(int slotid, int ctype, int meta) {
			if(this.world != null && this.world.isRemote()) {
				${JavaModName}.PACKET_HANDLER.sendToServer(new ${name}GUISlotChangedMessage(slotid, x, y, z, ctype, meta));
				${name}handleSlotAction(entity, slotid, ctype, meta, x, y, z);
			}
		}
		</#if>
	<#else>
	...
	@SubscribeEvent public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
		...
		if(event.phase == TickEvent.Phase.END && entity.openContainer instanceof ${name}GuiContainerMod) {
